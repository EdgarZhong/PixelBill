...
| **AI 自学习 P2** | ✅ **已完成** | **全链路对齐 v5.1 冻结口径：循环消费、渐进式 UI、生命周期联动** |
...
| **P2: BatchProcessor 循环消费（逐日清空队列）** | 2026-03-29 | 已完成（当前工作区） |
| **P2: renameCategory 回归冻结口径（只改名不重分类）** | 2026-03-29 | 已完成（当前工作区） |
| **P2: 渐进式标签 UI（ReclassifyConfirmDialog + SettingsPage 改造）** | 2026-03-29 | 已完成（当前工作区） |
| **P2: 生命周期联动补全（classify_memory / classify_examples / memory_snapshots）** | 2026-03-29 | 已完成（当前工作区） |
| **P2: 切换账本自动 recoverPending + enqueue 空日期防御** | 2026-03-29 | 已完成（当前工作区） |
| **P2: 全量路径锁定交易列表 + 当场解锁** | 2026-03-29 | 已完成（当前工作区） |
| **P2: 删除标签记忆文件失效标记** | 2026-03-29 | 已完成（当前工作区） |
...
| 任务 | 完成日期 | 提交 |
|------|----------|------|
| **前端剥离 Phase 1-5：全链路 Capacitor 解耦** | 2026-04-07 | b5ae38d |
| **Moni 整合 T1-T11：Memphis UI 全量接入** | 2026-04-08 | e53465b |
...
| Moni 整合后续联调：设置/记账/账本切换入口 | P1 | BottomNav 按钮尚未全部接入业务逻辑 |
| 前端剥离 Phase 6：IndexedDB 适配器 + 测试 | P2 | 纯 Web 环境支持 |
...
- **已完成的对齐补口**
  - 设置页已落地渐进式范围确认对话框，且不同范围按钮在点击当场完成对应的 dirtyDates 计算与入队。
  - 范围按钮入队成功后会自动通知消费端启动；若消费端已在运行，则不重复唤起。
  - `重命名标签` 已回归冻结口径：只批量改名，不进入渐进式重分类流程。
  - 生命周期联动已补齐到 `classify_memory`、`classify_examples`、`memory_snapshots` 与 `classify_queue`。
  - 全量路径已补齐锁定交易列表与当场解锁能力；删除标签时会追加记忆失效标记。
- **结论**
  - P2 的基础设施层、触发层、消费层与标签管理 UI/UX 已对齐当前冻结口径。
  - 当前阶段主工作已转入 P3；P2 仅保留后续回归维护与问题修复。
...
1. **渐进式标签 UI 已落地**
   ...
   - 全量路径已提供锁定交易列表与当场解锁能力。
2. **触发层与范围按钮已一一对应**
   - 每个范围按钮都在点击当场完成：前置处理 → dirtyDates 计算 → 入队。
   - 不再依赖延后推断、默认范围或后台补语义。
3. **消费端已自动衔接**
   - 范围按钮入队成功后自动通知消费端启动。
   - 若当前已在消费，不重复唤起，仅保留新任务等待循环消费。
4. **冻结口径已回归**
   - 重命名标签只做字符串改名；
   - 不触发重分类；
   - 不改动锁定状态。
...
6. **后续工作**
   - 继续保留 `runP2FullChainRegression()` 与边界回归作为回归基线；
   - 主开发重心转入 P3。
