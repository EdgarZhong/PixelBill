...
- 最新基线（2026-03-23）：Agent A 已验收通过，Agent B 正在接入“仅当前账本消费”

## 2.1 实施前基线确认（2026-03-23）

- 队列任务源为按账本隔离文件：`classify_queue/{ledger}.json`
- C 侧按任务消费时，必须显式携带并使用 `ledger` 上下文，不得默认回退到其他账本
- Prompt v5 结构保持不变，C 仅负责“取数 + 组装 + 回写链路稳定”
...
3. 日维度批次数据正确进入 `days`
4. 回写结果不破坏现有锁定保护语义
5. 在账本切换前后，不会把 A 账本任务拼装到 B 账本请求体
