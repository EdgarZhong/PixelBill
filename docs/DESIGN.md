...
*   **I5. P3 快速修正层 (移动端列表内联分类)**:
    *   **目标**: 在交易列表中直接完成分类修正，减少“进入详情页→返回”的路径损耗。
    *   **触发区域**: 仅分类标签可触发内联修正；点击交易其他区域仍进入详情页。
    *   **主流程**:
        *   用户点击列表项中的分类标签。
        *   弹出居中分类轮盘，选择后立即生效并关闭面板。
        *   底部浮现轻量提示条（3 秒自动消失），提供“补充备注”入口。
    *   **备注策略**:
        *   首次分类修正后默认不强制填写备注，允许零阻力完成主任务。
        *   用户主动点击“文字补充”时，再打开轻量输入层写入 `user_note`。
    *   **触发边界**:
        *   快速修正仅执行用户分类写入，不自动触发“按日期重分类入队”。
        *   分类队列自动触发边界保持不变：仅 CSV 导入允许自动触发，其它场景遵循显式确认触发。
